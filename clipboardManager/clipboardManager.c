...
int ensure_dirs(void) {
    ...
    if (stat(BASE_DIR, &st) != 0) {
        if (mkdir(BASE_DIR, 0755) != 0 && errno != EEXIST) {
            __err("mkdir base: %s", strerror(errno));
            __exit("ensure_dirs(): -1");
            return -1;
        }
    }
    if (stat(IMG_DIR, &st) != 0) {
        if (mkdir(IMG_DIR, 0755) != 0 && errno != EEXIST) {
            __err("mkdir imgs: %s", strerror(errno));
            __exit("ensure_dirs(): -1");
            return -1;
        }
    }
    __exit("ensure_dirs(): 0");
    return 0;
}
...
void write_nextid(unsigned int v) {
    __entry1("write_nextid(%d)", v);
    FILE *f = fopen(NEXTID_FILE, "w");
    if (!f) {
        __exit1("write_nextid(): !f");
        return;
    }
    fprintf(f, "%u\n", (v) % HISTORY_SIZE);
    fclose(f);
    __exit1("write_nextid()");
}

/* Append path to history file (one per line). Newest appended. */
int append_history_path(const char *path) {
    __entry1("append_history_path(path=%s)", path);
    FILE *f = fopen(HISTORY_FILE, "a");
    if (!f) {
        __err("fopen history append: %s", strerror(errno));
        __exit1("append_history_path(): -1");
        return -1;
    }
    fprintf(f, "%s\n", path);
    fclose(f);
    __exit1("append_history_path(): 0");
    return 0;
}
...
void build_data_filename(char *buf, size_t buflen, unsigned int id, const char *datatype, const char *ext) {
    __entry1("build_data_filename(id=%u, datatype=%s, ext=%s)", id, datatype, ext);
    char idbuf[16];
...
xcb_atom_t get_atom(xcb_connection_t *c, const char *name) {
    ...
    if (!r) {
        __exit("get_atom(): XCB_ATOM_NONE, !r");
...
int handle_poll_clipboard(xcb_connection_t *c, xcb_window_t win, unsigned int *nextid) {
            ...
            if (rep) {
                ...
                if (len > 0) {
                    ...
                    if ((size_t)len == last_len && crc == last_crc && target == last_target) {
                        free(rep);
                        __exit2("handle_poll_clipboard(): 0, duplicate");
                        return 0;
                    }
                    ...
                    if (save_text_gzip(fname, data, (size_t)len) == 0) {
                        ...
                        __log2("Saved TEXT (%d bytes) to %s", len, tmpath);
                        saved = 1;
                        last_crc = crc;
                        last_len = (size_t)len;
                        last_target = target;
            ...
            if (rep) {
                ...
                if (len > 0) {
                    ...
                    if (save_bytes(fname, data, (size_t)len) == 0) {
                        snprintf(tmpath, sizeof(tmpath), "%s", fname);
                        append_history_path(tmpath);
                        __log("Saved IMAGE (%s, %d bytes) to %s", ext, len, tmpath);
                        saved = 1;
                        last_crc = crc;
                        last_len = (size_t)len;
                        last_target = target;
    ...
    __exit2("handle_poll_clipboard(): %d", saved);
    return saved;
}

int restoreClipboardContent(xcb_connection_t *c, xcb_window_t win, const char *historyPath){
    __entry("restoreClipboardContent(path=%s)", historyPath);
    if (!historyPath) {
